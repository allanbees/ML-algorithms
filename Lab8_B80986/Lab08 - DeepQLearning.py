...
class Target(nn.Module):
    def __init__(self, in_features):
        super().__init__()
        self.target(in_features)
        
    def target(self, in_features):
        self.f1 = nn.Linear(in_features, 100)
    ...
    def forward(self, x):
        ...
        x = nn.ReLU(self.f2(x))
        x = nn.ReLU(self.f3(x))
        x = nn.Softmax(self.f4(x), dim=-1)
        return x

class Policy(nn.Module):
    def __init__(self, in_features):
        super().__init__()
        self.policy(in_features)
        
    def policy(self, in_features):
        self.f1 = nn.Linear(in_features, 100)
        self.f2 = nn.Linear(100, 20)
        self.f3 = nn.Linear(20, 50)
        self.f4 = nn.Linear(50, 3)
...
class Agent():
    ...
    def __init__(self, memory_capacity, batch_size, c_iters, learning_rate, discount_factor, eps_greedy, decay):
        ...
        self.decay = decay
        targetNet = Target().to(device)
        policyNet = Policy().to(device)
        opt_T = optim.Adam(targetNet.parameters(), lr=learning_rate)
        opt_P = optim.Adam(policyNet.parameters(), lr=learning_rate)
