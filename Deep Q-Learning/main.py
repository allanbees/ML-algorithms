...
class Agent():
    ...
    def __init__(self, memory_capacity, batch_size, c_iters, learning_rate, discount_factor, eps_greedy, decay):
        ...
        self.target = DQN(3 * WIDTH * HEIGHT, memory_capacity)
        self.policy = DQN(3 * WIDTH * HEIGHT, memory_capacity)
    ...
    def simulation(self, env):
        ...
        while not env.is_terminal_state():
            ...
            iters += 1
            counter_step += 1
            if iters == self.c_iters:
                self.target = self.policy.copy()
                iters = 0
            self.eps_greedy = 1 - counter_step * self.decay
    ...
    def step(self, env, learn=True):
        ...
        if random.uniform(0, 1) < self.eps_greedy:
            action = random.randint(0, 3)
        else:
            with torch.no_grad():
                state_old_tensor = torch.tensor(state_old.reshape(1, 3 * WIDTH * HEIGHT), dtype=torch.float32)
                prediction = self.policy(state_old_tensor)
                action = torch.argmax(prediction.detach()).item()

        reward, state_new = env.perform_action(action)
        
        if reward > 0:
            env.moves_left = INIT_MOVES
        
        if learn:
            self.policy.train_short_memory(state_old, action, state_new, reward)
